<?php
/**
 * Minimal inline Lucide-icon ports (stroke-based SVGs matching lucide-react's default
 * viewBox/stroke settings) for the small set actually used in hand-coded templates
 * (header/footer/sticky call bar). Icons referenced dynamically by name from ACF fields
 * (e.g. flexible-content "icon" sub-fields) go through hh_icon()/assets/icons/*.svg
 * instead (see inc/helpers.php) — this file is just for the fixed icons used directly
 * in PHP markup.
 */

const HH_LUCIDE_PATHS = [
    ...
    'clock' => '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16.5 14.25"/>',
    ...
    'package-check' => '<path d="M21 10V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l2-1.14"/><path d="m7.5 4.27 9 5.15"/><polyline points="3.29 7 12 12 20.71 7"/><line x1="12" x2="12" y1="22" y2="12"/><path d="m17 13.5 2 2 4-4"/>',
    ...
    // Added for page-about.php (Chapter 01-06 icon set) — sourced from lucide-static v1.23.0.
    ...
    // Added for single-service_category.php / single-service_subcategory.php /
    // template-parts/subcategory-tractor.php (Category.tsx, ServiceDetail.tsx,
    // SubCategory.tsx, TractorPage.tsx ports). Real path data pulled from
    // lucide-static@0.462.0 (matches this project's pinned lucide-react version).
    // NOTE: 'circle-check-big' is lucide-react's CheckCircle2 (a different icon than the
    // 'circle-check' already registered above) — used extensively across these templates.
    ...
    // Aliases so hh_icon()'s naive PascalCase->kebab conversion (no hyphen inserted before
    // a trailing digit) still resolves "CheckCircle2" / "Building2" if a future ACF
    // "icon" field value is typed using the original Lucide React component name.
    'check-circle2' => '<path d="M21.801 10A10 10 0 1 1 17 3.335"/><path d="m9 11 3 3L22 4"/>',
...
function hh_lucide(string $name, string $classes = 'w-5 h-5'): string {
    $inner = HH_LUCIDE_PATHS[$name] ?? HH_LUCIDE_PATHS['check'];
    return sprintf(
