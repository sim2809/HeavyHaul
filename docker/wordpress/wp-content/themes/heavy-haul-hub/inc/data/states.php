<?php
/**
 * Fixed US states reference data — NOT WordPress content (not editable in wp-admin),
 * ported verbatim from src/data/categories.ts's `states` array. Used to generate the
 * virtual /services/{category}/state/{state} pages (see inc/rewrite-rules.php).
 */

function hh_states(): array {
    static $states = null;
    if ($states === null) {
        $states = [
...
function hh_find_state(string $slug): ?array {
